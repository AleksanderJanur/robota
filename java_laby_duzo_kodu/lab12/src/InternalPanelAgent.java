...
public class InternalPanelAgent extends Thread {
    ...
    public void run(){
        for(;;){
            ...
            if (tmp == null) return;
            if (tmp.toFloor == elevatorCar.getFloor()) continue;
            if (tmp.toFloor > elevatorCar.getFloor()) {
                ElevatorStops.get().setLiftStopUp(tmp.toFloor);
            } else {
                ElevatorStops.get().setLiftStopDown(tmp.toFloor);
            }
        }
    }
}
